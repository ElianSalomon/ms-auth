...
public class AuthController {
    ...
    public ResponseEntity<AuthResponse> register(@RequestBody RegisterRequest request) {
        if (usuarioRepository.existsByEmail(request.getEmail())) {
            return ResponseEntity.status(HttpStatus.CONFLICT)
                    .body(new AuthResponse("El correo ya esta registrado", null, null, null, request.getEmail(), null));
        }
    ...
    public ResponseEntity<AuthResponse> login(@RequestBody LoginRequest request) {
        ...
        if (usuario == null || Boolean.FALSE.equals(usuario.getActivo()) || !usuario.getPassword().equals(request.getPassword())) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                    .body(new AuthResponse("Credenciales invalidas", null, null, null, request.getEmail(), null));
        }
